...
const CustomThemeEditor = ({ theme, onSave, onClose }) => {
  ...
  return (
    <div className="space-y-4 p-4">
      <h3 className="text-lg font-bold mb-4">Customize Theme Colors</h3>
      ...
      {Object.entries(customTheme).map(([key, value]) => (
        <div key={key} className="flex items-center gap-4">
          ...
          />
          <div className={`w-8 h-8 rounded ${value.split(' ')[0]}`} />
        </div>
      ))}
...
const LayoutEditor = ({ layout, onSave, onClose }) => {
  ...
  const handleSave = () => {
    localStorage.setItem('customLayout', JSON.stringify(customLayout));
    onSave(customLayout);
    onClose();
  };
  ...
  return (
    <div className="space-y-4 p-4">
      <h3 className="text-lg font-bold mb-4">Customize Layout</h3>
      
      {Object.entries(customLayout).map(([section, properties]) => (
        <div key={section} className="space-y-2">
          <h4 className="font-medium">{section}</h4>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm">Grid Column:</label>
              <input
                type="text"
                value={properties.gridColumn}
                onChange={(e) => handleLayoutChange(section, 'gridColumn', e.target.value)}
                className="w-full p-2 rounded bg-gray-800 text-white"
              />
            </div>
            <div>
              <label className="block text-sm">Grid Row:</label>
              <input
                type="text"
                value={properties.gridRow}
                onChange={(e) => handleLayoutChange(section, 'gridRow', e.target.value)}
                className="w-full p-2 rounded bg-gray-800 text-white"
              />
            </div>
          </div>
      ...
      <div className="min-h-screen bg-black/95 text-white p-8">
        ...
        {showSettings && (
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50">
            <div className={`${activeTheme.primary} rounded-lg p-6 max-w-2xl w-full`}>
              <div className="flex justify-between items-center mb-6">
                <div className="flex gap-4">
                  <button
                    onClick={() => setSettingsTab('theme')}
                    className={`flex items-center gap-2 px-4 py-2 rounded ${
                      settingsTab === 'theme' ? 'bg-gray-700' : ''
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`flex items-center gap-2 px-4 py-2 rounded ${
                      settingsTab === 'layout' ? 'bg-gray-700' : ''
                    }`}
                  >
